...
import { tetrisRes, TimeoutKey } from "./utils.ts";
...
export const    arcadeGame = () => {
	socket = new WebSocket(`ws://${address}/api/tetris/tetrisArcade`);
...
const   movePiece = (direction: string) => {
	...
	const   repeat = async () => {
		...
		postToApi(`http://${address}/api/tetris/movePiece`, { argument: arg, roomId: tetrisGameInfo.getGameId() });
	...
	const   keydownHandler = async (event: KeyboardEvent) => {
		...
		switch (key) {
			...
			case userKeys.getClockwizeRotate().toUpperCase():
				if (event.repeat)
					return ;
				postToApi(`http://${address}/api/tetris/rotatePiece`, { argument: "clockwise", roomId: tetrisGameInfo.getGameId() });
				return ;
			...
			case userKeys.getCountClockwizeRotate().toUpperCase():
				if (event.repeat)
					return ;
				postToApi(`http://${address}/api/tetris/rotatePiece`, { argument: "counter-clockwise", roomId: tetrisGameInfo.getGameId() });
				return ;
			...
			case userKeys.getRotate180().toUpperCase():
				if (event.repeat)
					return ;
				postToApi(`http://${address}/api/tetris/rotatePiece`, { argument: "180", roomId: tetrisGameInfo.getGameId() });
				return ;
			...
			case userKeys.getHardDrop().toUpperCase():
				if (event.repeat)
					return ;
				postToApi(`http://${address}/api/tetris/dropPiece`, { argument: "Hard", roomId: tetrisGameInfo.getGameId() });
				return ;
			...
			case userKeys.getSoftDrop().toUpperCase():
				...
				postToApi(`http://${address}/api/tetris/dropPiece`, { argument: "Soft", roomId: tetrisGameInfo.getGameId() });
				return ;
			...
			case userKeys.getHold().toUpperCase():
				...
				if (event.repeat)
					return ;
				postToApi(`http://${address}/api/tetris/holdPiece`, { argument: "hold", roomId: tetrisGameInfo.getGameId() });
				loadTetrisPage("board");
				return ;
	...
	const keyupHandler = async (event: KeyboardEvent) => {
		...
		switch (key) {
			...
			case userKeys.getSoftDrop().toUpperCase():
				...
				return postToApi(`http://${address}/api/tetris/dropPiece`, { argument: "Normal", roomId: tetrisGameInfo.getGameId() });
		}
	}
